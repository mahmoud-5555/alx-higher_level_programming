...
def matrix_divided(matrix, div):
	...
	#save the length of the first row to check if the rows have the same number of the element 
	length_first_row = len(matrix[0])
	for row in matrix:
		#check each row : its length must be equal to the length of the first row
